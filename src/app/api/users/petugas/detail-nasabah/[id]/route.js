...
export async function GET(request, { params }) {
  try {
    ...
    if (nasabah.bank_sampah_id !== user.bank_sampah_id) {
      return NextResponse.json(
        { message: "Nasabah bukan dari unit Anda" },
        { status: 403 },
      );
    }
    ...
    const allSetor = await prisma.transaksiSetor.findMany({
      where: { nasabah_id: nasabahId, is_cancelled: false },
    ...
    const allDetailSetor = await prisma.detailSetor.findMany({
      where: {
        transaksi: { nasabah_id: nasabahId, is_cancelled: false },
      },
      select: { berat: true },
    });
    ...
    const allTarik = await prisma.transaksiTarik.findMany({
      where: { nasabah_id: nasabahId, status: { not: "DIBATALKAN" } },
    ...
    const summary = {
      ...
      totalNilaiTarik: allTarik.reduce(
        (sum, t) => sum + Number(t.jumlah_tarik),
        0,
      ),
      totalBeratSampah: allDetailSetor.reduce(
        (sum, d) => sum + Number(d.berat),
        0,
      ),
      beratPerKategori,
    };
