...
export default function StatsGrid({ data, formatRupiah }) {
  ...
  // Menghitung rasio saldo aktif dari total uang masuk tabungan
  const uangMasukTabungan = perputaran_uang_per_metode?.TABUNG || 0;
  const rasioSaldoAktif =
    uangMasukTabungan > 0
      ? ((saldo_aktif / uangMasukTabungan) * 100).toFixed(1)
      : "0.0";
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-4 md:gap-6">
            ...
            >
              ...
              <span className="font-semibold text-gray-800 dark:text-white">
                ...
                <span className="text-xs text-gray-500">
                  ({gender_breakdown?.[key]?.persen || "0.00"}%)
                </span>
              </span>
            </div>
          ))}
          ...
          <div className="pt-3 border-t border-gray-100 dark:border-slate-800 space-y-1.5">
            <div className="flex justify-between items-center px-2 py-1 rounded-lg bg-green-50 dark:bg-green-900/20 text-xs">
              ...
              <span className="font-bold text-green-700">
                {formatRupiah(perputaran_uang_per_metode?.TABUNG || 0)}
              </span>
            </div>
            <div className="flex justify-between items-center px-2 py-1 rounded-lg bg-orange-50 dark:bg-orange-900/20 text-xs">
              ...
              <span className="font-bold text-orange-600">
                {formatRupiah(total_penarikan_rp)}
              </span>
            </div>
          </div>
        </div>
      ...
      {/* Saldo Aktif (Updated Version) */}
      <div className="p-6 rounded-2xl border border-gray-200 shadow-sm bg-white dark:bg-slate-900 dark:border-slate-700 flex flex-col justify-between">
        <div>
        ...
        <div className="mt-4 pt-4 border-t border-gray-200 dark:border-slate-700 space-y-3">
          ...
          <div className="w-full bg-gray-100 dark:bg-slate-800 rounded-full h-1.5 overflow-hidden">
            <div
              ...
              style={{
                width: `${Math.min(parseFloat(rasioSaldoAktif), 100)}%`,
              }}
            ></div>
          </div>
